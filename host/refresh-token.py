...
def push_token(private_key: str, app_id: int, installation_id: int, vm_ip: str):
    token = get_token(private_key, app_id, installation_id)
    subprocess.run(
        [
            "sshpass",
            "-e",
...
def main():
    ...
    parser.add_argument(
        "private_key", help="Path to the GitHub App private key PEM file"
    )
    ...
    parser.add_argument(
        "--installation-id", type=int, required=True, help="GitHub App installation ID"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--once", action="store_true", help="Push token once and exit")
    group.add_argument(
        "--continuous", action="store_true", help="Push token and keep refreshing"
    )
    ...
    if args.continuous:
        while True:
            ...
            try:
                push_token(
                    args.private_key, args.app_id, args.installation_id, args.vm_ip
                )
